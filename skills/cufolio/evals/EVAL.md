...
| 3. Skill performance benchmarks | `tests/test_skill_benchmarks.py` + `evals/benchmark_workflows.py` + `evals/thresholds.toml` | the SKILL.md workflows meet quantitative standards | Yes | No |

This file documents **Layer 2** (the NV-BASE agent evals). Layer 1 runs in normal CI; Layer 3 is
described in `benchmark_workflows.py` / `thresholds.toml`.
